...
func _physics_process(delta: float) -> void:
	...
	if is_zooming:
		zoom_target = clampf(zoom_target + zoom_direction * zoom_speed * delta, minimum_zoom, maximum_zoom)
	...
	if not is_equal_approx(display_zoom, zoom_destination):
		display_zoom = lerpf(display_zoom, zoom_destination, 1.0 - exp(-zoom_lerp_speed * delta))
